...
Logger::Logger(LogLevel level, int type , const std::string& file_name, const std::string& file_path, int max_size) : m_set_level(level), m_type(type) {

  if (m_type == 0) {
    return;
  }
  m_asnyc_logger = std::make_shared<AsyncLogger>(file_name , file_path, max_size);
}
...
void Logger::InitGlobalLogger(int type, const std::string& level,  const std::string& file_name, const std::string& file_path, int max_size) {


  LogLevel global_log_level = StringToLogLevel(level);
  printf("Init log level [%s]\n", LogLevelToString(global_log_level).c_str());
  g_logger = new Logger(global_log_level, type, file_name, file_path, max_size);
